...
FIELD_SETS: dict[str, list[str]] = {
    ...
    "reaction": ["id", "name", "substrate", "product", "substrate_coeff",
                 ...
                 "reversible"],
    "quantity": ["name", "expr"],
    "person": ["name", "age", "sex", "weight", "height", "ethnicity"],
    "trait": ["smoking", "pack_years", "alcohol", "exercise", "pregnant"],
    "disease": ["name", "category", "severity", "onset_age", "description"],
    "disease_gene": ["gene", "type", "activity"],
    "disease_metabolite": ["id", "type", "concentration", "normal"],
    "drug": ["name", "smiles", "formula", "mw", "drug_type", "target_protein",
             "binding_affinity_kd", "dose", "route", "interval", "duration",
             "bioavailability", "absorption_rate", "vd", "cl", "half_life",
             "hepatic_eh", "renal_fraction", "protein_binding",
             "cyp_metabolism", "transporter_affected", "non_cyp_metabolism"],
    "pd_effect": ["drug", "target", "type", "ec50", "emax", "hill"],
    "qsp_binding": ["drug", "kind", "kd_nM", "kss_nM", "emax",
                    "kd_agonist", "ki"],
...
LONG_TAIL_KINDS = [
    ...
    "synbio_design", "human", "virtual_patient",
]
...
FIELD_DOCS: dict[str, str] = {
    ...
    "expr": "Physical composition `A+B`/`A-B` (two `#type` terms or numbers).",
    ...
    "age": "Patient age in years (float, default 30).",
    "sex": "Patient sex: male | female.",
    "weight": "Body weight in kg (float, default 70).",
    "ethnicity": "Ethnicity (default european).",
    "smoking": "Smoking status: never | former | current.",
    "pack_years": "Smoking pack-years (float).",
    "alcohol": "Alcohol drinks per week (float).",
    "exercise": "Exercise level: sedentary | light | moderate | vigorous.",
    "pregnant": "bool: pregnant status.",
    "category": "Disease category (e.g. metabolic_overload).",
    "severity": "Severity 0.0–1.0.",
    "onset_age": "Age of disease onset (years).",
    "description": "Free-text disease description.",
    "activity": "Activity fraction 0.0–1.0.",
    "normal": "Normal concentration (mM).",
    "smiles": "SMILES string for the drug molecule.",
    "formula": "Molecular formula (e.g. C4H11N5).",
    "mw": "Molecular weight (g/mol; auto-inferred from SMILES).",
    "drug_type": "Drug molecule type: small_molecule | biologic | antibody | peptide.",
    "binding_affinity_kd": "Binding affinity Kd (nM).",
    "dose": "Dose in mg.",
    "route": "Administration route: oral | iv | im | sc.",
    "interval": "Dosing interval in hours (default 24).",
    "bioavailability": "Bioavailability fraction (auto-inferred from SMILES).",
    "absorption_rate": "Absorption rate constant (auto-inferred).",
    "vd": "Volume of distribution in L (auto-inferred).",
    "cl": "Clearance in mL/min (auto-inferred).",
    "half_life": "Half-life in hours (auto-inferred).",
    "hepatic_eh": "Hepatic extraction ratio (auto-inferred).",
    "renal_fraction": "Renal elimination fraction (auto-inferred).",
    "protein_binding": "Protein binding fraction (auto-inferred).",
    "cyp_metabolism": "CYP enzyme metabolism (e.g. CYP3A4:0.5,CYP2D6:0.3).",
    "transporter_affected": "Transporter effects (e.g. SLCO1B1:0.6).",
    "non_cyp_metabolism": "Non-CYP metabolism (e.g. UGT1A1:0.7).",
    "ec50": "EC50 for PD effect (µM).",
    "emax": "Maximum effect fraction 0.0–1.0.",
    "hill": "Hill coefficient for dose-response.",
    "kd_nM": "Dissociation constant for QSP binding (nM).",
    "kss_nM": "Steady-state Kss for QSP binding (nM).",
    "kd_agonist": "Kd for agonist binding (nM).",
    "ki": "Inhibition constant Ki (nM).",
    "axis": "Endocrine axis: diabetes | addison | hypothyroid | stress.",
    "level": "Hormone level offset.",
    "infection_severity": "Infection severity 0.0–1.0.",
    "autoimmune_activation": "Autoimmune activation level 0.0–1.0.",
    "immunosuppression": "Immunosuppression level 0.0–1.0.",
}
